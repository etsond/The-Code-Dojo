...
function toQueryString(obj) {
  ...
  // Helper function to recursively build query string
  const buildQueryString = (value, prefix) => {
    if (Array.isArray(value)) {
      // If value is an array, append each array item with the same key
      value.forEach(item => {
        buildQueryString(item, prefix);
      });
    } else if (typeof value === 'object') {
      // If value is an object, recursively build query string with nested keys
      for (const key in value) {
        if (value.hasOwnProperty(key)) {
          buildQueryString(value[key], prefix ? `${prefix}.${key}` : key);
        }
      }
    } else {
      // If value is a primitive type, append key-value pair to query string
      result.push(`${prefix}=${encodeURIComponent(value)}`);
    }
  }
